...
SAVE_ROOT="${SAVE_ROOT:-results/observations/rolekv_ragged_subset_gpu2_core6_q5}"
...
ROLE_MIN_VOTES="${ROLE_MIN_VOTES:-1}"
SEED="${SEED:-0}"
...
for MODE in ${MODES}; do
  ...
  mkdir -p "${SAVE_DIR}"
  echo "[run_rolekv_ragged_subset_gpu2] mode=${MODE} save_dir=${SAVE_DIR}"
